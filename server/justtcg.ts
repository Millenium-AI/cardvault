...
export function extractPrice(
  ...
  if (tcgplayerSkuId) {
    // Find the exact variant by SKU ID — never use variants[0] which may be wrong condition
    const v = variants.find(
      (v: any) => String(v.tcgplayerSkuId) === String(tcgplayerSkuId)
    ) ?? variants.find(
      (v: any) => v.condition === condition
    ) ?? variants[0];
...
export async function batchFetchPrices(
  ...
  try {
    ...
    for (const item of toFetch) {
      ...
      const priceResult = extractPrice(card, item.condition, item.printing, item.tcgplayerSkuId ?? null);
      if (!priceResult) {
        stillNeedsPricing.push(item);
        continue;
      }
...
// ── Single-item price fetch (used by price-history route) ────────────────────
export async function fetchSinglePrice(
  ...
  try {
    ...
    return extractPrice(card, condition, printing, tcgplayerSkuId ?? null);
  } catch (err: any) {
